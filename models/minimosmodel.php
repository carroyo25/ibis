        ...
        function listarMinimosPaginado($parametros, $page = 1, $limit = 5){
            ...
            try {
                ...
                /*SELECT
                    ...
                    FORMAT(SUM(e.cant_ingr), 2) AS ingresos,
                    MAX(u.cabrevia) AS cabrevia,
                    FORMAT(COALESCE((SELECT SUM(cc.cantsalida) FROM alm_consumo cc WHERE cc.ncostos = c.idcostos AND cc.idprod = e.codprod AND cc.flgactivo = 1), 0),2) AS consumos,
                    DATE_FORMAT(MAX(m2.ffecha), '%d/%m/%Y') AS ffecha,
                    FORMAT(MAX(m2.ntotal), 2) AS ntotal
                ...
                LEFT JOIN cm_producto p ON e.codprod = p.id_cprod
                LEFT JOIN tb_unimed u ON p.nund = u.ncodmed
                LEFT JOIN (SELECT m.idprod, m.ncostos, m.ffecha, m.ntotal FROM alm_minimo m ORDER BY m.ffecha DESC) m2 ON m2.idprod = e.codprod AND m2.ncostos = c.idcostos
