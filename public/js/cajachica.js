$(function(){
    ...
    $(".lista,.lista_grid").on("click",'a', function (e) {
        ...
        }else if(contenedor_padre == "lista_entidad"){
    ...
    $("#saveItem").click(function (e) { 
        ...
        try {
            ...
            if (result['codigo_entidad'] == '') throw new Error("Elija el nombre de proveedor");
            ...
            if ( accion == 'n' ){
                $.post(RUTA+"cajachica/nuevoPedido", {cabecera:result,detalles:JSON.stringify(itemsSave())},
                    function (data, textStatus, jqXHR) {
                ...
                $.post(RUTA+"cajachica/modificaPedido", {cabecera:result,detalles:JSON.stringify(itemsSave())},
                    function (data, textStatus, jqXHR) {
                        mostrarMensaje(data.mensaje,data.clase);
                        accion = "u";
                        grabado = true;
    ...
    $("#tablaDetalles tbody").on('keypress','input', function(e) {
        if (e.which == 13) {
            try {
                ...
                let precio = $(this).parent().parent().find("td").eq(7).children().val();
                ...
                $("#tablaDetalles tbody  > tr").each(function () {
                    suma += parseFloat($(this).find('td').eq(8).text()||0,10);
                })
