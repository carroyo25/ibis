        ...
        fetch(RUTA + "activos/asignados", {
          ...
          .then((data) => {
            if (data.asignado) {
                ...
                document.getElementById("cargo").value = data.datos[0]["cargo"].toUpperCase();
                document.getElementById("nombres").value = data.datos[0]["nombres"] + " " + data.datos[0]["paterno"] + " " + data.datos[0]["materno"];
                ...
                document.getElementById("ubicacion").style.backgroundColor = '#ffebee';
            }else{
...
function calcularVencimiento() {
    ...
    if (fechaRevision) {
        const fecha = new Date(fechaRevision);
        let diasASumar = periodo === 'anual' ? 365 : 180;
        fecha.setDate(fecha.getDate() + diasASumar);
