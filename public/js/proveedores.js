$(async function () {
  ...
  $.post(
    RUTA + "proveedores/obtenerValores",
    { bancos: "02", tipo: "03" },
    ...
    try {
      ...
      if (accion == "n") {
        ...
        if (permisos[0]["modifica"] != null) {
          $.post(
            RUTA + "proveedores/modificaEntidad",
            {
              datos: result,
              contactos: JSON.stringify(obtenerContactos()),
              bancos: JSON.stringify(obtenerBancos()),
          ...
          mostrarMensaje(
            "No tiene acceso a modificar proveedores",
            "mensaje_correcto",
          );
        }
      }
  ...
  $("#closeProcess").click(function (e) {
    ...
    $.post(
      RUTA + "proveedores/actualizaListado",
      function (data, textStatus, jqXHR) {
        $(".lista").hide();
  ...
  $("#tablaPrincipal tbody").on("click", "tr", function (e) {
    ...
    $.post(
      ...
      function (data, textStatus, jqXHR) {
        ...
        $(
          "input[name=agente][value='" + data.proveedor[0].nagenret + "']",
        ).prop("checked", true);
  ...
  $("#btnAceptarPregunta").click(function (e) {
    ...
    $.post(
      RUTA + "proveedores/desactivaProveedor",
      { id: index },
      function (data, textStatus, jqXHR) {
        $("#tablaPrincipal tbody").empty().append(data);
  ...
  $("#contactos").on("click", "a", function (e) {
    ...
    if ($(this).attr("href") == "#") {
      ...
      $.post(
        RUTA + "proveedores/quitarItem",
        {
          query:
            "UPDATE cm_entidadcon SET cm_entidadcon.nflgactivo =:estado WHERE cm_entidadcon.nidcontact =:id",
          id: $(this).attr("href"),
        },
  ...
  $("#bancos").on("click", "a", function (e) {
    ...
    if ($(this).attr("href") == "#") {
      ...
      $.post(
        ...
        {
          query:
            "UPDATE cm_entidadbco SET cm_entidadbco.nflgactivo =:estado WHERE cm_entidadbco.nitem =:id",
          id: $(this).attr("href"),
        },
        function (data, text, requestXHR) {
          fila.remove();
