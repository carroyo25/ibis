$(function () {
      ...
      fetch(RUTA + "registroti/datosNombre", {
        ...
        .then((data) => {
          if (data.registrado) {
            ...
            $("#firma_barra").attr("src", data.firma);

            apellidosfirma =
              data.datos[0].paterno + " " + data.datos[0].materno;
              nombrefirma = data.datos[0].nombres;

            $("#tablaPrincipal tbody").empty().append(data.anteriores);
          } else {
  ...
  $("#btnAceptarDialogoKardex").click(function (e) {
    ...
    try {
      ...
      if ($("#cantidad_dialogo").val() == "")
        throw new Error("Ingrese una cantidad para registrar");
      if ($("#serie_dialogo").val() == "")
        throw new Error("Debe registrar la serie del usuario");

      let cant = $("#cantidad_dialogo").val(),
        fsalida = fechaActual(),
  ...
  $("#btnRegister").click(function (e) {
    ...
    if ($("#docident").val() == "") {
      mostrarMensaje(
        "Ingrese el nombre o el documento del colaborardor",
        "mensaje_error",
      );
      return false;
    }
    ...
    $.post(
      RUTA + "registroti/buscaCatalogo",
      { tipo: 37 },
      function (data, textStatus, jqXHR) {
        $("#tablaKardex tbody").empty().append(data);
  ...
  $("#btnFirmaCorreo").click((e) => {
    ...
    fetch(RUTA + "registroti/firma", {
      ...
      .then((data) => {
        if (data.success){
          $("#firma").attr("src", data.archivo + "?v=" + new Date().getTime());
          mostrarMensaje("Archivo de firma creado","mensaje_correcto");
        }else{
          mostrarMensaje("Error al crear firma","mensaje_error");
        }
      });
  ...
  $("#send_signature").click(function(e){
    e.preventDefault();
  ...
  $("#btnAceptarEnviar").click(function(e){
    e.preventDefault();

    const firmaImg = document.getElementById('firma_barra');
    const firmaBase64 = firmaImg.src; // Ya debería ser base64

    const formData = new FormData();
    formData.append("nombre",$("#nombre").val());
    formData.append("dni",$("#docident").val());
    formData.append("destino",$("#email_destino").val());
    formData.append("correo_tecnico",$("#mail_user").val());
    formData.append("nombre_tecnico",$("#name_user").val());
    formData.append("copia",$("#enviar_copia").prop('checked'));
    formData.append('firma_imagen', firmaBase64);
    

    fetch(RUTA+'registroti/correo_firma',{
      method:'POST',
      body:formData
    })
    .then(response=>response.json())
    .then(data=>{
      if (data.succes){
        mostrarMensaje(data.message,"mensaje_correcto");
      }else{
  ...
  $("#btnCancelarEnviar").click(function(e){
    e.preventDefault();

    $("#enviar_firma").fadeOut();
