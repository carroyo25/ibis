$(function () {
  ...
  renderizarParametros(32, "condicion_entrega");
  ...
  $("#tablaPrincipal tbody").on("click", "tr", function (e) {
    ...
    $.post(
      ...
      function (data, textStatus, jqXHR) {
        ...
        $("#condicion_entrega").val(data.cabecera[0].nCondicion);
        $("#puntoEntrega").val(data.cabecera[0].cPuntoEntrega);
        $("#fecha_compromiso").val(data.cabecera[0].fCompromiso);
        ...
        if (data.cabecera[0].nigv != 0) {
          igv = parseFloat(data.cabecera[0].total_multiplicado) * 0.18;
        ...
        let total_orden =
            parseFloat(total) + parseFloat(igv) + parseFloat(adicionales),
          total_orden_format = formatoNumeroConComas(total_orden, 2, ".", ",");

        $("#it").val(total_orden_format);

        $("#estado").removeClass().addClass(estado);

        $("#tablaDetalles tbody").empty().append(data.detalles);

        $("#tablaComentarios tbody").empty().append(data.comentarios);

        $("#tablaAdicionales tbody").empty().append(data.adicionales);
        ...
        if (data.bocadillo != 0) {
          $(".button__comment").text(data.bocadillo).show();
        } else {
  ...
  $("#tablaPrincipal tbody").on("click", "a", function (e) {
    ...
    if (accion == "descargar") {
      try {
        ...
        if (atencion == 47 && firmas)
          throw new Error("La orden esta en firmas");
      ...
      try {
        if ($(this).parent().parent().data("estado") == 59)
          throw "La orden esta en firmas.";
  ...
  $("#nuevoRegistro").click(function (e) {
    ...
    $("#estado").removeClass().addClass("textoCentro estado procesando");
  ...
  $(".mostrarLista").focus(function (e) {
    ...
    if ($("#codigo_estado").val() == 59) return false;
  ...
  $(".lista,.lista_grid").on("click", "a", function (e) {
    ...
    if (contenedor_padre == "listaAlmacen") {
      ...
      $.post(
        RUTA + "orden/detallesEntidad",
        { codigo: $(this).data("ruc") },
        function (data, textStatus, jqXHR) {
    ...
    $.post(
      ...
      function (data, textStatus, jqXHR) {
        $("#esperar").fadeOut(function (e) {
          $("#busqueda").fadeIn();
          $("#pedidos tbody").empty().append(data);
        });
      },
      "text",
    );
  ...
  $("#pedidos tbody").on("click", "tr", function (e) {
    ...
    try {
      if (costos != $(this).data("costos"))
        throw "El item esta otro centro de costos";
      ...
      if (!checkExistTable($("#tablaDetalles tbody tr"), codigo, 5)) {
        ...
        $.post(
          RUTA + "orden/marcaItem",
          { id: $(this).data("iditem"), estado: 1, io: $(this).data("itord") },
  ...
  $("#btnAceptItems").click(function (e) {
    ...
    try {
      if ($("#tablaDetalles tbody").length == 0)
        throw "No se selecciono ningún item";

      $.post(
        RUTA + "orden/datosPedido",
        { pep: pedido, prof: proforma, ent: entidad },
        function (data, textStatus, jqXHR) {
          ...
          if ($("#numero").val() === "") $("#numero").val(data.orden);
  ...
  $("#saveOrden").click(function (e) {
    ...
    try {
      ...
      if (result["total"] == "")
        throw new Error("No se registro el total de la orden");
      if ($("#tablaDetalles tbody tr").length <= 0)
        throw new Error("No tiene items cargados");
  ...
  $("#btnAceptarAprueba").click((e) => {
    ...
    $.post(
      RUTA + "orden/aprobacion",
      { cabecera: result, detalles: JSON.stringify(detalles()) },
      function (data, text, requestXHR) {
        mostrarMensaje(data.mensaje, data.clase);
    ...
    try {
      ...
      if (
        ($("#procura").val() != 1 ||
          $("#finanzas").val() != 1 ||
          $("#operaciones").val() != 1) &&
        $("#nivel_autorizacion").val() === "47"
      )
        throw "La orden no ha sido autorizada";
      ...
      $.post(
        RUTA + "orden/envioOrden",
        { cabecera: result, detalles: JSON.stringify(detalles()) },
        function (data, textStatus, jqXHR) {
          mostrarMensaje(data.mensaje, data.clase);
          $("#tablaPrincipal tbody").empty().append(data.ordenes);
          $("#esperar").fadeOut();
        },
        "json",
      );
  ...
  $("#tablaDetalles tbody").on("click", "a", function (e) {
    ...
    $.post(
      RUTA + "orden/marcaItem",
      {
        id: $(this).parent().parent().data("itped"),
  ...
  $("#tablaDetalles tbody").on("click", ".consultaPrecios", function (e) {
    ...
    $.post(
      RUTA + "firmas/precios",
      {
        codigo: $(this).parent().data("codprod"),
        descripcion: $(this).parent().data("descrip"),
      },
      function (data, text, requestXHR) {
        $("#tablaPrecios tbody").empty().append(data);
  ...
  $("#uploadCotiz").click(function (e) {
    ...
    try {
      if ($("#codigo_orden").val().length == 0)
        throw new Error("Por favor grabe la orden");

      if (parseInt($("#atach_counter").text()) > 0) {
        $.post(
          RUTA + "orden/listarAdjuntos",
          { orden: $("#codigo_orden").val(), tipo: "ORD" },
          function (data, text, requestXHR) {
            $(".listaArchivos").empty().append(data.adjuntos);
            $("#archivos").fadeIn();
          },
          "json",
        );
  ...
  $("#itemCostos").change(function (e) {
    ...
    let body = document.getElementById("detalles_pedidos");

    // Loading con colspan correcto (5 columnas)
    body.innerHTML = `<tr class="cargando" style="text-align:center;background:#fff">
                          <td colspan="11">
                            <div style="display:grid;color:#5f6368;">
                              <i class="fas fa-spinner fa-spin" style="font-size:40px;padding:2rem"></i><br> Cargando...
                            </div>
                          </td>
                        </tr>`;

    $.post(
      RUTA + "orden/ItemsPorCostos",
      { costo: $(this).val() },
      function (data, textStatus, jqXHR) {
        $("#pedidos tbody").empty().append(data);
      },
  ...
  $("#btnConfirmAdic").on("click", function (e) {
    ...
    $("#total_adicional").val(
      sumarAdicionales($("#tablaAdicionales tbody >tr"), 2),
    );
  ...
  $("#addAdic").click(function (e) {
    ...
    $("#tablaAdicionales tbody").append(row);
  ...
  $('input[name="radioIgv"]').change(function (e) {
    e.preventDefault();
    if ($("#total_numero").val() > 0) calcularTotales();
  ...
  $("#condicion_entrega").on("change", function () {
    const texto = $(this).val() == -1 ? '':$("#condicion_entrega option:selected").text();
    $("#condicion_entrega_texto").val(texto);
  });

  function renderizarParametros(clase, select) {
    let opcion = '<option value="-1">Seleccione Opción</option>';

    const lista = document.getElementById(select);

    if (!lista) {
      console.warn("No se encontró el select:", select);
      return;
    }
    ...
    fetch(RUTA + "orden/parametros", {
      method: "POST",
      body: formData,
    })
      .then((response) => response.json())
      .then((data) => {
        // Validar que data.datos exista y sea array
        if (data && data.datos && Array.isArray(data.datos)) {
          data.datos.forEach((element) => {
            opcion += `<option value="${element.nidreg}">${element.cdescripcion}</option>`;
          });
        }

        // ✅ innerHTML es una propiedad, no una función
        lista.innerHTML = opcion;
      })
      .catch((error) => {
        console.error("Error al cargar parámetros:", error);
        lista.innerHTML = '<option value="-1">Error al cargar</option>';
      });
  }
});
...
actualizarFila = (entidad, concepto, importe_neto, moneda, fila) => {
  ...
  $("#" + fila)
    .find("td")
    .eq("7")
    .text(simbolo_moneda + importe_neto);
};
...
sumarAdicionales = (TABLA, indice) => {
  ...
  TABLA.each(function () {
    sum += parseFloat(
      $(this).find("td").eq(indice).children().val().replace(/,/g, ""),
      10,
    );
  });
...
calcularTotales = () => {
  ...
  if ($("#total_numero").val() == 0) {
    ...
    im =
      parseFloat($("#total_numero").val()) *
      parseFloat($('input[name="radioIgv"]:checked').val());
  }
...
sumardias = () => {
  ...
  if (fecha.getDay() === 0 || fecha.getDay() === 6) {
    ...
    mostrarMensaje(
      "La fecha de entrega se cambio un dia utíl",
      "mensaje_correcto",
    );
  }
  ...
  fecha =
    fecha.getFullYear() +
    "-" +
    $.strPad(fecha.getMonth() + 1, 2) +
    "-" +
    $.strPad(fecha.getDate(), 2);
