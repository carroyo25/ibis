...
<?php
    ...
    class Model{
            ...
            try {
                $sql = $this->db->connect()->prepare("SELECT
                                                    ...
                                                    FROM
                                                        ...
                                                        LEFT JOIN
                                                    ibis.tb_proyectos ON ibis.tb_pedidocab.idcostos = ibis.tb_proyectos.nidreg
                                                    LEFT JOIN ibis.tb_area ON ibis.tb_pedidocab.idarea = ibis.tb_area.ncodarea
                                                    LEFT JOIN ibis.tb_parametros ON ibis.tb_pedidocab.idtrans = ibis.tb_parametros.nidreg
                                                    LEFT JOIN ibis.tb_parametros AS transportes ON ibis.tb_pedidocab.idtrans = transportes.nidreg
                                                    LEFT JOIN ibis.tb_parametros AS estados ON ibis.tb_pedidocab.estadodoc = estados.nidreg
                                                    LEFT JOIN ibis.tb_parametros AS tipos ON ibis.tb_pedidocab.idtipomov = tipos.nidreg
                                                    LEFT JOIN ibis.tb_user ON ibis.tb_pedidocab.usuario = ibis.tb_user.iduser
                                                    LEFT JOIN ibis.tb_partidas ON ibis.tb_pedidocab.idpartida = ibis.tb_partidas.idreg 
                                                    WHERE
                                                        tb_pedidocab.idreg = :id 
                                                        AND tb_pedidocab.estadodoc BETWEEN :min 
        ...
        public function consultarOrdenId($id){
            try {
                $sql = $this->db->connect()->prepare("SELECT
                                                        ...
                                                        o.cnumcot,
                                                        o.userModifica,
                                                        UPPER(o.cObservacion) AS concepto,
                                                        UPPER(p.detalle) AS detalle,
                                                        p.docPdfAprob,
                                                        UPPER(CONCAT_WS(' ', py.ccodproy, py.cdesproy)) AS costos,
                                                        o.ncodpry,
                                                        o.ncodalm,
                                                        UPPER(CONCAT_WS(' ', a.ccodarea, a.cdesarea)) AS area,
                                                        o.ncodmon,
                                                        mon.cdescripcion AS nombre_moneda,
                                                        mon.cabrevia AS abrevia_moneda,
                                                        o.ntipmov,
                                                        tip.cdescripcion AS tipo,
                                                        pag.cdescripcion AS pagos,
                                                        o.ffechaent,
                                                        est.cabrevia AS estado,
                                                        est.cdescripcion AS descripcion_estado,
                                                        e.crazonsoc,
                                                        e.cnumdoc,
                                                        UPPER(ec.cnombres) AS cnombres,
                                                        ec.cemail,
                                                        ec.ctelefono1,
                                                        tra.cdescripcion AS transporte,
                                                        UPPER(al.cdesalm) AS cdesalm,
                                                        UPPER(al.ctipovia) AS direccion,
                                                        e.cviadireccion,
                                                        e.cemail AS mail_entidad,
                                                        e.nagenret,
                                                        o.cverificacion,
                                                        o.ntotal,
                                                        o.nigv,
                                                        UPPER(o.lentrega) AS lentrega,
                                                        o.cReferencia,
                                                        FORMAT(o.ntotal, 2) AS ctotal,
                                                        p.nivelAten,
                                                        o.nNivAten AS autorizado,
                                                        o.nfirmaLog,
                                                        o.nfirmaFin,
                                                        o.nfirmaOpe,
                                                    ...
                                                    FROM
                                                        ...
                                                        LEFT JOIN tb_proyectos py ON o.ncodcos = py.nidreg
                                                        LEFT JOIN tb_area a ON o.ncodarea = a.ncodarea
                                                        LEFT JOIN tb_parametros mon ON o.ncodmon = mon.nidreg 
                                                        LEFT JOIN tb_parametros tip ON o.ntipmov = tip.nidreg 
                                                        LEFT JOIN tb_parametros pag ON o.ncodpago = pag.nidreg 
                                                        LEFT JOIN tb_parametros est ON o.nEstadoDoc = est.nidreg 
        ...
        /*para generar la vista de los previas*/
        public function generarDescargaOrden($id){
           require_once("public/formatos/ordenes.php");
        ...
        public function kardexAnterior($d,$c){
            try {
                $salida = "";
                $sql = $this->db->connect()->prepare("SELECT
                                                        alm_consumo.idreg,
